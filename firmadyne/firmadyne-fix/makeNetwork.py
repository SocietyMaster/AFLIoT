...
%(QEMU_ENV_VARS)s ${QEMU} -m 256 -M ${QEMU_MACHINE} -kernel ${KERNEL} \\
    ...
    -nographic \\
    %(QEMU_NETWORK)s -serial file:${WORK_DIR}/qemu.final.serial.log -serial unix:/tmp/qemu.${IID}.S1,server,nowait -monitor unix:/tmp/qemu.${IID},server,nowait -display none 
"""
