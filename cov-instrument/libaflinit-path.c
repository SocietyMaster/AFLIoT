...
u32  __afl_path_initial[MAX_PATH_SIZE];
u32* __afl_path_ptr = __afl_path_initial;

u32 __afl_path_index_initial;
u32* __afl_path_index_ptr = &__afl_path_index_initial;

u32 __afl_shm_pointers[3];
...
static void __afl_map_shm(void) {
    ...
    if (id_str) {
        ...
        /* Get shm_path from afl */

        u8 *path_id_str = (u8 *)getenv(SHM_PATH_ENV_VAR);

        u32 path_shm_id = atoi((const char *)path_id_str);

        __afl_path_ptr = shmat(path_shm_id, NULL, 0);

        if (__afl_path_ptr == (void *)-1) _exit(1);

        /* Get shm path index from afl */

        u8 *path_index_id_str = (u8 *)getenv(SHM_PATH_INDEX_ENV_VAR);

        u32 path_index_shm_id = atoi((const char *)path_index_id_str);

        __afl_path_index_ptr = shmat(path_index_shm_id, NULL, 0);

        if (__afl_path_index_ptr == (void *)-1) _exit(1);
    }
...
u32 * afl_manual_init() {
    ...
    __afl_shm_pointers[1] = (u32)__afl_path_ptr;
    __afl_shm_pointers[2] = (u32)__afl_path_index_ptr;
