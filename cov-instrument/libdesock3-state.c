...
#define DESOCK_STATE_FILE_ENV "DESOCK_STATE_FILE"
...
#define DESOCK_BUFFER_SIZE 0x1000
...
__attribute__((constructor)) void desock_hook_init() {
	...
	char *state_file = (char *)getenv(DESOCK_STATE_FILE_ENV);
	if (!state_file) {
		perror("desock init error: state file path needed");
		fflush(stdout);
		_exit(0);
	}

	struct stat state_file_stat;
	if (stat(state_file, &state_file_stat) == -1) {
		perror("desock init error: unable to stat state file");
		fflush(stdout);
		_exit(0);
	}

	state_buffer = (char *)malloc(state_file_stat.st_size);
	if (!state_buffer) {
		perror("desock init error: unable to alloc memory");
		fflush(stdout);
		_exit(0);
	}

	FILE *state_file_fd = fopen(state_file, "rb");
	fread(state_buffer, 1, state_file_stat.st_size, state_file_fd);
	fclose(state_file_fd);

	state_file_count = 0;
	int state_buffer_offset = 0;
	while (1) {
		int length = *(int *)(state_buffer + state_buffer_offset);
		if (length == 0xffffffff) {
			break;
		}
		if (length <= 0) {
			perror("desock init error: invalid state file 1");
			fflush(stdout);
			_exit(0);
		}
		state_buffer_offset += sizeof(int) + length;
		if (state_buffer_offset >= state_file_stat.st_size) {
			perror("desock init error: invalid state file 2");
			fflush(stdout);
			_exit(0);
		}
		state_file_count++;
...
void desock_socket_sync_loop_output(int from, int to) {

	DESOCK_DEBUG("starting forwarding from %d to %d!\n", from, to);

	char buffer[DESOCK_BUFFER_SIZE];
	int count = state_file_count;
	while (count--) {
		while (state_mutex == 1) {
			usleep(10);
		}

		int ret = read(from, buffer, DESOCK_BUFFER_SIZE);
		if (ret <= 0) {
			perror("desock_socket_sync_loop_output: error read data");
			fflush(stdout);
			_exit(0);
		}
...
void desock_socket_sync_loop_input(int from, int to) {
	...
	while (count--) {
		...
		char *data_buffer = state_buffer + state_buffer_offset + sizeof(int);
		write(to, data_buffer, length);
...
void *desock_socket_sync_thread(void *fd) {
	...
	if (from == 0) {
		desock_socket_sync_loop_input(from, to);
	} else {
		desock_socket_sync_loop_output(from, to);
	}
