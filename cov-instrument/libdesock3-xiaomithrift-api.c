...
/* Xiaomi Thrift Environments */

#define DESOCK_XIAOMI_THRIFT_METHOD_ENV "DESOCK_XIAOMI_THRIFT_METHOD"
#define DESOCK_XIAOMI_THRIFT_API_ID_ENV "DESOCK_XIAOMI_THRIFT_API_ID"
...
/* Xiaomi thrift packet */

typedef struct xiaomi_thrift_packet
{
    unsigned int total_length;
    unsigned short thrift_version;
    ...
    char *data;
    unsigned char stop;
} xiaomi_thrift_packet_t;

xiaomi_thrift_packet_t packet;
unsigned char thrift_packet_buffer[0x1000];
unsigned char thrift_data_buffer[0x800];
unsigned int thrift_api_id;
...
__attribute__((constructor)) void desock_hook_init() {
	...
	/* Xiaomi Thrift Packet Init */

	packet.thrift_version = 0x8001;
    packet.message_type = 0x0001;
    packet.seq_id = 0;
    packet.field_id = 0x0001;
    packet.data_type = 0x0b;
    packet.stop = 0;
    memset(thrift_packet_buffer, 0, sizeof(thrift_packet_buffer));

	packet.method = getenv(DESOCK_XIAOMI_THRIFT_METHOD_ENV);
	if (!packet.method) {
		perror("desock init error: xiaomi thrift method needed");
		fflush(stdout);
		_exit(0);
	}
	packet.method_length = strlen(packet.method);

	char * thrift_api_id_literal = getenv(DESOCK_XIAOMI_THRIFT_API_ID_ENV);
	if (!thrift_api_id_literal) {
		perror("desock init error: xiaomi thrift api id needed");
		fflush(stdout);
		_exit(0);
	}
	thrift_api_id = strtol(thrift_api_id_literal, NULL, 10);
}
...
void write_packet_buffer(){
	  unsigned char *ptr = thrift_packet_buffer;
		*(unsigned int *)ptr = htonl(packet.total_length);
		ptr += 4;
		*(unsigned short *)ptr = htons(packet.thrift_version);
		ptr += 2;
		*(unsigned short *)ptr = htons(packet.message_type);
		ptr += 2;
		*(unsigned int *)ptr = htonl(packet.method_length);
		ptr += 4;
		memcpy(ptr, packet.method, packet.method_length);
		ptr += packet.method_length;
		*(unsigned int *)ptr = htonl(packet.seq_id);
		ptr += 4;
		*ptr = packet.data_type;
		ptr++;
		*(unsigned short *)ptr = htons(packet.field_id);
		ptr += 2;
		*(unsigned int *)ptr = htonl(packet.data_length);
		ptr += 4;
		memcpy(ptr, packet.data, packet.data_length);
		ptr += packet.data_length;
		*ptr = packet.stop;
}
...
int desock_socket_sync(int from, int to, int timeout) {
	...
	if(from == 0){
		int length = 0, length_done = 0;
		memset(thrift_data_buffer, 0, sizeof(thrift_data_buffer));
		sprintf(thrift_data_buffer, "{ \"api\": %d, ", thrift_api_id);
		length = strlen(thrift_data_buffer);
		memcpy(thrift_data_buffer + length, buffer, ret);
		length += ret;
		sprintf(thrift_data_buffer + length, "}");
		length += 1;
		
		packet.data = thrift_data_buffer;
		packet.data_length = length;
		packet.total_length = sizeof(packet.thrift_version) + sizeof(packet.message_type) + sizeof(packet.method_length) + packet.method_length + sizeof(packet.seq_id) + sizeof(packet.data_type) + sizeof(packet.field_id) + sizeof(packet.data_length) + packet.data_length + sizeof(packet.stop);
		length = packet.total_length + sizeof(packet.total_length);

		write_packet_buffer();

		while (length_done != length) {
			ret = write(to, thrift_packet_buffer + length_done, length - length_done);
