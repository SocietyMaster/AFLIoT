...
u8  __afl_cov_area_initial[MAP_SIZE];
u8* __afl_cov_area_ptr = __afl_cov_area_initial;
...
static void __afl_map_shm(void) {
    ...
    if (id_str) {
        ...
        /* Get coverage map from parent */

        u8 *cov_id_str = (u8 *)getenv(SHM_COV_ENV_VAR);

        u32 cov_shm_id = atoi((const char *)cov_id_str);

        __afl_cov_area_ptr = shmat(cov_shm_id, NULL, 0);

        if (__afl_cov_area_ptr == (void *)-1) _exit(1);
    }
...
u32 * afl_manual_init() {
    ...
    __afl_shm_pointers[1] = (u32)__afl_cov_area_ptr;
